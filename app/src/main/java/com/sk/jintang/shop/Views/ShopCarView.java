...
import static com.sk.jintang.shop.MainShopActivity.carAdapter;


public class ShopCarView extends FrameLayout {
	private TextView car_limit, tv_amount,text_shipping_free;
	...
	protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
		...
		if (iv_shop_car == null) {
			...
			text_shipping_free =  findViewById(R.id.text_shipping_free);
	...
	public void updateAmount(BigDecimal amount,int startPs,int shippingFree) {
		if (amount.compareTo(new BigDecimal(0.0)) == 0) {
			car_limit.setText("¥"+startPs+" 起送");
			...
			behavior.setState(BottomSheetBehavior.STATE_COLLAPSED);
		} else if (amount.compareTo(new BigDecimal(startPs)) < 0) {
			car_limit.setText("还差 ¥" + new BigDecimal(startPs).subtract(amount) + " 起送");
			car_limit.setTextColor(Color.parseColor("#a8a8a8"));
			car_limit.setBackgroundColor(Color.parseColor("#535353"));
		...
		if(shippingFree==0){
			text_shipping_free.setText("配送免费");
		}else{
			text_shipping_free.setText("配送费¥"+shippingFree+"元");
		}
	...
	public void showBadge(int total) {
		if (total > 0) {
			...
			text_shipping_free.setVisibility(View.VISIBLE);
		} else {
			text_shipping_free.setVisibility(View.INVISIBLE);
