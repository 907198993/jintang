...
public class XianShiNewFragment extends BaseFragment implements LoadMoreAdapter.OnLoadMoreListener{
    ...
    public static XianShiNewFragment newInstance(int timeId,long beginTime,long endTime) {
        Bundle args = new Bundle();
        args.putInt(Constant.timeId, timeId);
        args.putLong(Constant.beginTime, beginTime);
        args.putLong(Constant.endTime, endTime);
        XianShiNewFragment fragment = new XianShiNewFragment();
    ...
    protected void initView() {
        ...
        adapter=new LoadMoreAdapter<XianShiQiangGouObj>(mContext,R.layout.item_xian_shi_qiang,pageSize) {
            ...
            public void bindData(LoadMoreViewHolder holder, int position, XianShiQiangGouObj bean) {
                ...
                //还没开始
                if(new Date().getTime()<getArguments().getLong(Constant.beginTime)){
                ...
                }else if(new Date().getTime()>getArguments().getLong(Constant.endTime)){ //已结束
...
//                        BigDecimal divide = bd1.divide(bd2).multiply(new BigDecimal(100));
    ...
    public void getData(int page, boolean isLoad) {
        ...
        map.put("period_time_id",getArguments().getInt(Constant.timeId)+"");
        map.put("page", page+"");
