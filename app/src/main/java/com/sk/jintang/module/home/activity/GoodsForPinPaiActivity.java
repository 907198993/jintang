...
public class GoodsForPinPaiActivity extends BaseActivity implements LoadMoreAdapter.OnLoadMoreListener{
    ...
    protected void initView() {
        pinPaiId = getIntent().getStringExtra(Constant.IParam.pinPaiId);
    ...
    private void getData(int page,boolean isLoad) {
        ...
        map.put("brand_id",pinPaiId);
        map.put("page",page+"");
        map.put("pagesize",pageSize+"");
        map.put("user_id",getUserId());
        map.put("sign",GetSign.getSign(map));
        ApiRequest.goodsForPP(map, new MyCallBack<List<GoodsSearchObj>>(mContext,pl_load,pcfl) {
            @Override
