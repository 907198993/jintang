...
public class GoodsEvaluateFragment extends BaseFragment implements LoadMoreAdapter.OnLoadMoreListener{
    ...
    private List<ArrayList>imgItemZJList=new ArrayList<>();
    ...
    public static GoodsEvaluateFragment newInstance(GoodsDetailObj obj) {
            ...
            public void bindData(LoadMoreViewHolder holder, int position, GoodsEvaluateObj bean) {
                ...
                if (notEmpty(bean.getImg_list())) {
                    for (int i = 0; i < bean.getImg_list().size(); i++) {
                        ...
                        Glide.with(mContext).load(imgUrl).error(R.color.c_press).into(evaluateImg);
                        int finalI = i;
                ...
                View ll_zhui_ping = holder.getView(R.id.ll_zhui_ping);
                List<GoodsEvaluateObj.AfterReviewBean> afterReviewList = bean.getAfter_review();
                if(notEmpty(afterReviewList)){
                    GoodsEvaluateObj.AfterReviewBean afterReview = afterReviewList.get(0);
                    ll_zhui_ping.setVisibility(View.VISIBLE);
                    holder.setText(R.id.tv_item_goods_evaluate_content_zj, afterReview.getContent());

                    LinearLayout ll_item_goods_evaluate_zj = (LinearLayout) holder.getView(R.id.ll_item_goods_evaluate_zj);
                    ll_item_goods_evaluate_zj.removeAllViews();
                    ArrayList<String>photoZJList=new ArrayList<>();
                    if (notEmpty(afterReview.getImg_list())) {
                        for (int i = 0; i < afterReview.getImg_list().size(); i++) {
                            String imgUrl = afterReview.getImg_list().get(i).getImg();
                            photoZJList.add(imgUrl);
                            ImageView evaluateImg = new ImageView(mContext);
                            evaluateImg.setScaleType(ImageView.ScaleType.FIT_CENTER);
                            LinearLayout.LayoutParams layoutParams = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT);
                            layoutParams.width = imgWidth;
                            ...
                            evaluateImg.setOnClickListener(new MyOnClickListener() {
                                ...
                                protected void onNoDoubleClick(View view) {
                                    Intent intent=new Intent();
                                    intent.putExtra(Constant.IParam.imgIndex, finalI);
                                    intent.putExtra(Constant.IParam.imgEvaluate, afterReview.getContent());
                                    intent.putStringArrayListExtra(Constant.IParam.imgList,imgItemZJList.get(position));
                                    STActivity(intent,ImageDetailActivity.class);
                                }
                            });
                            ll_item_goods_evaluate_zj.addView(evaluateImg);
                        }
                    }
                    imgItemZJList.add(photoZJList);
                }else{
                    ll_zhui_ping.setVisibility(View.GONE);
                }
            }
        };
