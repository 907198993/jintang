...
public class RichTextEditor extends ScrollView {
    ...
    public RichTextEditor(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        allLayout.setPadding(5, 5, 5, 5);//设置间距，防止生成图片时文字太靠边，不能用margin，否则有黑边
//        allLayout.setPadding(50, 15, 50, 15);//设置间距，防止生成图片时文字太靠边，不能用margin，否则有黑边
    ...
    public void addImageViewAtIndex(final int index, String imagePath) {
        ...
        Glide.with(getContext()).load(imagePath).crossFade().fitCenter().into(imageView);
        imageView.setAbsolutePath(imagePath);//保留这句，后面保存数据会用
        imageView.setScaleType(ImageView.ScaleType.FIT_CENTER);//Glide上面还有设置  裁剪剧中
